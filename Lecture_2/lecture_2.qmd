...
make_plate <- function(crs, lon_min=-180, lon_max=180, lat_min=-90, lat_max=90,
                       ind_radius=500000, ortho=FALSE) {
  sf_use_s2(FALSE)
  bb <- st_bbox(c(xmin=lon_min, ymin=lat_min, xmax=lon_max, ymax=lat_max),
                crs=st_crs(4326))

  land <- st_simplify(st_transform(st_make_valid(st_crop(world, bb)), crs),
                      dTolerance=10000)
  grat <- st_transform(st_crop(
            st_graticule(lon=seq(-180,180,30), lat=seq(-90,90,30), crs=st_crs(4326)),
            bb), crs)

  ctr <- expand.grid(lon=seq(-150,150,60), lat=seq(-60,60,30))
  ctr <- ctr[ctr$lon>=lon_min & ctr$lon<=lon_max &
             ctr$lat>=lat_min & ctr$lat<=lat_max, ]
  ctr <- st_as_sf(ctr, coords=c("lon","lat"), crs=4326)
  sf_use_s2(TRUE); cir <- st_buffer(ctr, ind_radius); sf_use_s2(FALSE)
  cir <- st_transform(cir, crs)

  if (ortho) {
    land <- land[!st_is_empty(land), ]
    grat <- grat[!st_is_empty(grat), ]
    cir  <- cir[!st_is_empty(cir), ]
  }

  lim <- st_bbox(grat)
  ggplot() +
    geom_sf(data=land, fill="#4e5767", colour="gray", linewidth=0.1) +
    geom_sf(data=cir,  fill="#C4704A", colour="#8a3f22", linewidth=0.3, alpha=0.7) +
    geom_sf(data=grat, colour="#42affa", linewidth=0.3) +
    coord_sf(xlim=c(lim["xmin"],lim["xmax"]), ylim=c(lim["ymin"],lim["ymax"]),
             expand=FALSE) +
    theme_void() +
...
make_plate("+proj=merc +lon_0=0 +datum=WGS84 +units=m +no_defs",
...
Conic (obviously)---preserves area while distorting shape
...
make_plate("+proj=aea +lat_1=29.5 +lat_2=45.5 +lon_0=-96 +datum=WGS84 +units=m +no_defs",
           lon_min=-170, lon_max=-30, lat_min=-40, lat_max=80, ind_radius=800000)
```

## Lambert Conformal Conic
Conic but preserves shape (and angle)
...
make_plate("+proj=lcc +lat_1=33 +lat_2=45 +lon_0=-96 +datum=WGS84 +units=m +no_defs",
           lon_min=-170, lon_max=-30, lat_min=-40, lat_max=75, ind_radius=800000)
```
...
make_plate("+proj=laea +lat_0=40 +lon_0=-100 +datum=WGS84 +units=m +no_defs",
           lon_min=-180, lon_max=20, lat_min=-45, lat_max=80, ind_radius=800000)
```
